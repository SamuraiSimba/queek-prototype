...
// ============================================================
// QUEEK ONBOARDING — Universal Access
// First Principles: Show capability, not proxies for capability
    ...
    <div style={{ textAlign: "center", paddingTop: 40 }}>
      ...
      <FadeIn delay={1100}>
        <p style={{
          ...
          No guessing if you're "the right fit."
          <br /><br />
...
function StepEmail({ name, onNext }) {
  ...
  return (
    <div>
      <FadeIn delay={200}>
        ...
        <p style={{ fontSize: 15, color: C.inkSoft, marginBottom: 36, lineHeight: 1.6 }}>
          We'll notify you when employers want to interview you.
          No spam, ever.
        </p>
      </FadeIn>
      ...
      <FadeIn delay={500}>
        <input
          ...
          onKeyDown={(e) => e.key === "Enter" && isValidEmail && onNext(email.trim())}
          ...
          style={{
            ...
            color: C.ink,
            background: C.white,
            border: `2px solid ${isValidEmail ? C.gold : C.border}`,
            borderRadius: 14,
            outline: "none",
            boxSizing: "border-box",
            transition: "border-color 0.3s ease",
            boxShadow: isValidEmail ? `0 0 0 4px ${C.goldGlow}` : "none",
            marginBottom: 24,
          }}
        />

        <GoldButton onClick={() => onNext(email.trim())} disabled={!isValidEmail}>
          Continue →
        </GoldButton>
      </FadeIn>
    </div>
...
function StepTruth({ name, onNext }) {
  ...
  useEffect(() => {
    const t = setTimeout(() => setRead(true), 3000);
    return () => clearTimeout(t);
  }, []);
    ...
    <div>
      <FadeIn delay={200}>
        ...
        <h2 style={{
          ...
          {name}, here's what's broken
          <br />
          <span style={{ color: C.gold }}>about hiring.</span>
        </h2>
      </FadeIn>
      ...
      <FadeIn delay={700}>
        <div style={{
          ...
          <p style={{ marginBottom: 16 }}>
            Employers try to answer: <strong style={{ color: C.ink }}>"Can you do this job?"</strong>
          </p>
          <p style={{ marginBottom: 16 }}>
            But they don't see you <em>do</em> the job.
          </p>
          <p style={{ marginBottom: 16 }}>
            They see:
          </p>
          <div style={{ paddingLeft: 20, marginBottom: 16 }}>
            <div style={{ marginBottom: 8 }}>→ Your resume (words you wrote about yourself)</div>
            <div style={{ marginBottom: 8 }}>→ Your school name (a proxy for intelligence)</div>
            <div style={{ marginBottom: 8 }}>→ Your previous job titles (a proxy for skill)</div>
            <div>→ Your name (a proxy for... nothing relevant)</div>
          </div>
          <p style={{ marginBottom: 0 }}>
            <strong style={{ color: C.ink }}>These aren't proof. They're guesses.</strong>
            <br />And guesses encode bias.
          </p>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={1400}>
        <div style={{
          ...
          <strong style={{ color: C.gold }}>Queek works differently.</strong>
          {" "}Employers see your verified skills, your match score, and your rate.
          Your name? Revealed only after they want to interview you.
          <br /><br />
          <strong>Because if you can do the job, that's all that should matter.</strong>
        </div>
      </FadeIn>
  ...
  return (
    <div>
      ...
      <FadeIn delay={400}>
        ...
        <GoldButton
          onClick={() => onNext(selected.map(id => SKILL_OPTIONS.find(sk => sk.id === id)?.label || id))}
...
function StepReady({ name, email, skills, rate }) {
        ...
        <>
          ...
          <FadeIn delay={700}>
            <GoldButton onClick={async () => {
              // Save candidate to Firestore with email
              ...
              alert("✅ Profile saved! You're now in the matching pool.");
            }}>
              Show me my matches →
            </GoldButton>
          </FadeIn>
...
function CandidateOnboarding() {
  ...
  const [data, setData] = useState({ name: "", email: "", skills: [], rate: 25 });

  const goNext = (payload) => {
    if (step === 1) setData((d) => ({ ...d, name: payload }));
    if (step === 2) setData((d) => ({ ...d, email: payload }));
    if (step === 4) setData((d) => ({ ...d, skills: payload }));
    if (step === 5) setData((d) => ({ ...d, rate: payload }));
    setStep((s) => s + 1);
  };
  ...
  const screens = [
    ...
    <StepEmail name={data.name} onNext={goNext} />,
    ...
    <StepReady name={data.name} email={data.email} skills={data.skills} rate={data.rate} />,
  ];
